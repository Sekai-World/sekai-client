...
Production Git publishing uses the `sekai-client-virtono-jp` GitHub App instead
...
uv run python deployment/github-app/install.py \
  --private-key /root/private/sekai-client-virtono-jp.private-key.pem
```

The installer copies the key to
`/root/.config/sekai-github-app/private-key.pem`, writes the non-secret App
