...
- The current `/root/pm2/sharedApiClientTW.yaml` and PM2 process description have
...
kubectl -n pjsk-account exec deployment/pjsk-account-service -- \
  python -c "import urllib.request; print(urllib.request.urlopen( \
  'http://127.0.0.1:8000/metrics').read().decode())" \
...
envsubst < deployment/pm2/canary/sharedApiClientTW.yaml.example \
  > /root/pm2/sharedApiClientTW.remote-canary.yaml
chmod 600 /root/pm2/sharedApiClientTW.remote-canary.yaml
```

Before applying it, confirm all placeholders were replaced, the bind remains
`127.0.0.1:39391`, `--workers 1` remains present, and no
