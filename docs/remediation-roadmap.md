...
| 2026-08-22 | 5 | KR is selected as the next rollout region. Inventory and a lease-scoped token are not yet prepared, so no KR production activation has been performed. | Complete KR preparation against the full rollout gate (inventory, lease-scoped token, region-specific configuration, pre-activation evidence, observation window); then run a one-region KR canary. |
