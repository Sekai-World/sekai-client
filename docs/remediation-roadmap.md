...
| 2026-08-22 | 5 | KR is selected as the next rollout region. Inventory and a lease-scoped token are not yet prepared, so no KR production activation has been performed. | Complete KR inventory/token preparation and the pre-activation rollback review; then run a one-region KR canary. |
