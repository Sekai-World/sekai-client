...
def enqueue_job(job: Callable[[], Any]) -> tuple[
    Optional[queue.Queue[Any]],
    Optional[JSONRPCInternalError]
]:
    """
...
def get_answer(
    ...
    try:
        ...
        return JSONRPCInternalError(
            data='Timed out waiting for worker response'
        )
...
def get_account_info() -> dict[str, Any]:
    ...
    if client_region in ("jp", "en"):
        filepath = path.join(
            dirname,
            f'sharedAccount.{client_region}.yaml'
        )
        if path.exists(filepath):
            ...
            logger.warning(
                'no %s account found, registering a new one',
                client_region
            )
    ...
    if client_region in ("cn", "tw", "kr"):
        access_token = getenv(
            f"SEKAI_{client_region.upper()}_ACCESS_TOKEN",
            None
        )
        sdk_open_id = getenv(
            f"SEKAI_{client_region.upper()}_SDK_OPEN_ID",
            None
        )
        if not access_token or not sdk_open_id:
            raise ValueError(
                f"Missing access token and/or SDK open id "
                f"for {client_region} server"
            )
        return {
            "loginInfo": {
...
def init(region: str) -> bool:
    ...
    logger.info(
        "Initialized API client for %s server",
        client_region
    )
    return True
...
@api.dispatcher.add_method
def check_versions(
    input_ver_info: Optional[dict[str, Any]] = None
) -> Any:
    """
...
@api.dispatcher.add_method
def login_user_info() -> Optional[dict[str, Any]]:
...
def fetch_user_event_ranking(target_user_id: str, event_id: int) -> Any:
    ...
    return run_job(
        lambda: api_client.fetch_user_event_ranking(
