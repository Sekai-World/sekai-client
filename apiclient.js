...
if (!existsSync('./account.yaml')) {
  logger.warn('no account.yaml found, created empty one, remember to fill GitHubToken!')
  writeFileSync('./account.yaml', yaml.safeDump({ userId: null, signature: null, credential: null, GitHubToken: null }))
}
...
async function refreshVersions() {
  ...
  logger.info(`appVersion ${appVersion} dataVersion ${dataVersion} assetVersion ${assetVersion} assetHash ${assetHash}`)
  ...
  await writeFile(path.join(masterDBDiffDir, 'versions.json'), JSON.stringify({appVersion, dataVersion, assetVersion, assetHash}, null, 2))
  ...
  for (let key in master) {
    await writeFile(path.join(masterDBDiffDir, `${key}.json`), JSON.stringify(master[key], null, 2));
    await git.add({fs, dir: masterDBDiffDir, filepath: `${key}.json`})
  }

  logger.debug('commit and push master db diff')
  await git.commit({ fs, dir: masterDBDiffDir, message: `master version ${dataVersion} asset version ${assetVersion}`, author: { name: 'master-db-diff-bot', email: 'anonymous@example.com' }})
  await git.push({ fs, http, dir: masterDBDiffDir, remote: 'origin', ref: 'main', onAuth: () => ({ username: GitHubToken }) })
}
...
async function bootstrap() {
  ...
  if (userTutorial.tutorialStatus !== 'end') {
    ...
    for (let status of steps.slice(steps.indexOf(userTutorial.tutorialStatus)+1)) {
      await doReq(`/user/${userId}/tutorial`, "patch", {
